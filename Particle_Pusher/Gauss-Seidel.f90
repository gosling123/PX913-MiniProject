MODULE GAUSS_SEIDEL !Module to handle claculation of phi, E etc.
  ...
  SUBROUTINE get_grid(n_x, n_y, grid_X, grid_Y, d_x, d_y)

    INTEGER, INTENT(IN) :: n_x, n_y !Number of grid elemnts on axis
    REAL(KIND=REAL64), DIMENSION(:), ALLOCATABLE :: grid_X, grid_Y
    REAL(KIND=REAL64), INTENT(OUT) :: d_x, d_y !Grid spacing on each axis
  ...
  SUBROUTINE get_charge_density(n_x, n_y, problem, rho)
    ...
    REAL(KIND=REAL64) :: d_x, d_y 
      
    CALL get_grid(n_x, n_y, grid_X, grid_Y, d_x, d_y)
    ...
    ELSE IF (problem .EQ. "single") THEN
      ...
      DO j=1, n_y
        DO i=1, n_x
          ...
          exponent_0 = ((grid_X(i)/0.1)**2 + (grid_Y(j)/0.1)**2)
  ...
  SUBROUTINE get_potential(n_x, n_y, phi, problem, gs_iter)
    ...
    REAL(KIND=REAL64) :: d_x, d_y, x_part, y_part, denominator
  ...
  FUNCTION get_electric_field_x(n_x, n_y, phi)
    ...
    DO j=1, n_y
      DO i=1, n_x
        ...
        E_x(i,j) = (phi(i+1,j) - phi(i-1,j))/(2*d_x) !Electric field in x
  ...
  FUNCTION get_electric_field_y(n_x, n_y, phi)
    ...
    DO i=1, n_x
      DO j=1, n_y
        ...
        E_y(i,j) = (phi(i,j+1) - phi(i,j-1))/(2*d_y) !Electric field in y
